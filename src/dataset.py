...
class LocalAudioSandboxDataset(Dataset):
    def __init__(self, data_dir=os.path.join("..", "data", "matrices")):
        self.data_dir = data_dir
        self.matrix_files = glob.glob(os.path.join(data_dir, "*.npy"))
        if not self.matrix_files:
            raise FileNotFoundError(
                f"No processed data found. "
                f"Please run 'python 1_generate_cqt.py' first"
            )
            
        # Load and stack all the individual file matrices into one massive timeline
        all_hops = []
        for file_path in self.matrix_files:
            matrix = np.load(file_path) # Shape: (84, total_hops)
            all_hops.append(matrix.T) # transpose to shape (total_hops, 84) so each row is one slice of time
    ...
    def __getitem__(self, idx):
        tensor_x = torch.from_numpy(self.data[idx]) # fetch a single slice and convert to a PyTorch tensor
        return tensor_x, tensor_x # self-supervised learning (target 'y' is the exact same as input 'X')
